...
object LineOfSight {
  ...
  def lineOfSight(input: Array[Float], output: Array[Float]): Unit = {
    ...
    while (idx < input.length) {
      maximum = maximum.max(input(idx) / idx)
      output(idx) = maximum
      idx += 1
    }
  }
  ...
  def upsweepSequential(input: Array[Float], from: Int, until: Int): Float = {
    ...
    var maximum = 0F
    while(idx < until) {
      maximum = maximum.max(if (idx==0) 0 else input(idx) / idx)
      idx += 1
    }
    maximum
  }
  ...
  def downsweepSequential(input: Array[Float], output: Array[Float],
    ...
    while(idx < until) {
      val angle = if (idx == 0) 0 else input(idx)/idx
  ...
  def downsweep(input: Array[Float], output: Array[Float], startingAngle: Float,
    ...
    tree match {
      ...
      case Node(left, right) => {
        val (_,_) = parallel(downsweep(input, output, startingAngle, left), downsweep(input, output, startingAngle.max(left.maxPrevious), right))
      }
    }
  }
